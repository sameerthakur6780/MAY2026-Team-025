...
import { Upload, Camera, Sparkles, CheckCircle2, Bell } from "lucide-react";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2 border-soft shadow-none">
          <CardContent className="p-6">
            ...
            <label htmlFor="photo-upload" data-testid="upload-label" className="block cursor-pointer border-2 border-dashed border-soft rounded-2xl p-10 text-center hover:border-forest transition-colors bg-canvas">
              ...
              <div className="font-display text-lg font-semibold text-[#1C1C1C]">
                {photoName ? photoName : "Click to upload classroom photo"}
              </div>
              <div className="text-sm text-[#5C5C5C] mt-1">JPG or PNG. AI face-recognition will identify present students.</div>
            </label>

            <div className="mt-6 flex flex-wrap items-center gap-3">
              <Button data-testid="run-ai-btn" onClick={runAI} disabled={processing} className="bg-forest hover:bg-[#162D24] text-white gap-2 rounded-full px-6">
                <Sparkles className="w-4 h-4" /> {processing ? "Analysing…" : "Run AI attendance"}
              </Button>
              {detected.length > 0 && (
                <Button data-testid="notify-parents-btn" onClick={notify} variant="outline" className="gap-2 rounded-full border-terracotta text-terracotta hover:bg-terracotta hover:text-white">
        ...
        <Card className="border-soft shadow-none">
          <CardContent className="p-6">
            <div className="text-xs tracking-[0.2em] uppercase font-bold text-[#5C5C5C]">How it works</div>
            <ol className="mt-4 space-y-4 text-sm text-[#1C1C1C]">
              <li className="flex gap-3"><span className="w-6 h-6 rounded-full bg-sage flex items-center justify-center text-forest font-bold text-xs shrink-0">1</span> Snap a wide photo of your batch.</li>
              <li className="flex gap-3"><span className="w-6 h-6 rounded-full bg-sage flex items-center justify-center text-forest font-bold text-xs shrink-0">2</span> AI matches faces to enrolled students.</li>
              <li className="flex gap-3"><span className="w-6 h-6 rounded-full bg-sage flex items-center justify-center text-forest font-bold text-xs shrink-0">3</span> Push safety notification to parents.</li>
            </ol>
          </CardContent>
        </Card>
      </div>
        ...
        <Card className="border-soft shadow-none mt-6 animate-fade-in-up">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <div className="text-sm text-forest font-medium inline-flex items-center gap-1.5">
              ...
              {detected.map(s => (
                <div key={s.id} data-testid={`detect-${s.id}`} className="flex items-center justify-between p-3 rounded-lg border border-soft">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-sage flex items-center justify-center text-forest font-semibold text-sm">{s.name[0]}</div>
                    <div>
                      <div className="text-sm font-medium">{s.name}</div>
                      <div className="text-xs text-[#5C5C5C]">{s.id}</div>
                    </div>
                  </div>
                  <Badge className={`border-0 ${s.present ? "bg-sage/60 text-forest" : "bg-[#FBD5D5] text-[#B23A48]"}`}>
