      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
        <Card className="border-soft shadow-none bg-yellow/15">
          <CardContent className="p-5 flex items-center gap-4">
            <div className="w-10 h-10 rounded-lg bg-yellow/30 flex items-center justify-center"><Clock className="w-5 h-5 text-yellow" /></div>
            <div>
        ...
        <Card className="border-soft shadow-none bg-sage/30">
          <CardContent className="p-5 flex items-center gap-4">
            <div className="w-10 h-10 rounded-lg bg-sage/70 flex items-center justify-center"><CheckCircle2 className="w-5 h-5 text-ink" /></div>
            <div>
        ...
        {items.map(h => (
          <Card key={h.id} data-testid={`stud-hw-${h.id}`} className="border-soft shadow-none">
            <CardContent className="p-5 flex flex-wrap items-center gap-4">
              <div className="w-11 h-11 rounded-xl bg-sage/60 flex items-center justify-center shrink-0"><BookOpen className="w-5 h-5 text-ink" /></div>
              <div className="flex-1 min-w-0">
                <div className="font-medium text-foreground">{h.title}</div>
                <div className="text-xs text-muted-foreground mt-0.5">Due {h.due}</div>
                {h.mySubmission && (
                  <div className="text-xs text-lime mt-1 inline-flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> {h.mySubmission}</div>
                )}
              </div>
              {h.mySubmission ? (
                <Badge className="bg-lime text-ink border-0">AI-verified</Badge>
              ) : (
                <Button data-testid={`submit-hw-${h.id}`} onClick={() => setDialog(h)} className="bg-coral hover:bg-coral-deep text-ink gap-2 rounded-full">
      ...
      <Dialog open={!!dialog} onOpenChange={o => { if (!o) setDialog(null); }}>
        <DialogContent>
          ...
          <label className="block cursor-pointer border-2 border-dashed border-soft rounded-xl p-8 text-center hover:border-coral bg-canvas mt-2">
            ...
            <div className="text-xs text-muted-foreground mt-1">Photo, PDF or DOC</div>
          </label>
          <Button data-testid="submit-hw-confirm" onClick={submit} className="w-full bg-coral hover:bg-coral-deep text-ink">Submit for verification</Button>
        </DialogContent>
      </Dialog>
    </DashboardLayout>
  );
