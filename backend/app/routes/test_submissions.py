...
test_submissions_bp = Blueprint("test_submissions", __name__, url_prefix="/api/test-submissions")
...
@test_submissions_bp.get("")
@role_required(*_ALL_ROLES)
def list_submissions():
    test_id, err = _int_arg("test_id")
    if err:
        return jsonify({"error": "validation_error", "message": {"test_id": [err]}}), 400
    ...
    role = get_jwt()["role"]
    query = list_submissions_query(role, test_id=test_id, student_id=student_id, status=status)
    return jsonify(paginate_query(query, serialize_submission)), 200


@test_submissions_bp.get("/<int:submission_id>/download")
@role_required(*_ALL_ROLES)
